...
4. **Save** a Markdown file to your Obsidian vault with YAML frontmatter, the cleaned transcript, the raw transcript, and an embedded link to the audio

The output note links back to your daily note (`YYYY-MM-DD.md`), so voice memos show up in Obsidian's backlinks panel and graph view.
...
If you want a cloud LLM for better cleanup (Claude, OpenAI, Gemini), the transcript text gets sent to the API but the audio never does. No account, no cloud sync, no analytics.{% sidenote 1 "Inspired by Steph Ango's '[file over app](https://stephango.com/file-over-app)'---your data should outlive any application." %}
...
Coati is [open source on GitHub](https://github.com/westonwestenborg/coati), or you can join the TestFlight [here](https://testflight.apple.com/join/9qK8RVbS).
